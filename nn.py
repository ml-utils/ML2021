...
class layer:
    ...
    def __init__(self, fan_in, fan_out, activation, strtin_range, act_parameters=(1, 1)):
        ...
        self.weights = strtin_range*(2*np.random.Generator.random([fan_out, fan_in+1])-1)
        self.weights /= fan_in
    ...
    def calc_local_gradient(self, error_signal):
        ...
        p_grandient = error_signal*self.derivative(self.latest_net)
        self.delta_weights += np.outer(p_gradient, self.latest_in)
        output_error_signal = np.matmul(p_grandient, self.weights)
...
class neuralnet:
    ...
    def __init__(self, units=(2, 2), activation, eta = 0.01, alpha = 0.5, lamda = 0.2, mb=20, **kwargs):
        self.layers = []

        for i in range(len(units)-1):
            lay = layer(unit[i], unit[i+1], activation, 0.1)
